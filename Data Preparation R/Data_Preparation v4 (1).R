...
remove<-names(df2) %in% c("acc_MILEPT","acc_LATITUDE", "acc_LONGITUD", "acc_TWAY_ID", "acc_TWAY_ID2", "acc_RAIL",
                          ...
                          "acc_MINUTE","acc_NOT_MIN","acc_ARR_MIN","acc_HOSP_MN")
...
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("acc_aux_STATE_nm","Rural","Urban", "Var1", "total", "per_rur"))]
...
#####################
##Find number of accidents by state and rural vs urban
acc_COUNTYall<-data.frame(table(df2.cat$acc_COUNTY,df2.cat$acc_aux_A_RU_nm))
...
#create same column name
acc_COUNTY.fatal$acc_COUNTY<-acc_COUNTY.fatal$Var1

#merge dataframes to pull in per_rur
df2.cat<-merge(df2.cat, acc_COUNTY.fatal, by ="acc_COUNTY" )

#find quartiles for % rural
summary(df2.cat$per_rur)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#0.0000  0.4276  0.5163  0.5203  0.6089  1.0000

df2.cat$acc_COUNTY_bin<-cut(df2.cat$per_rur, breaks=c(0.0000,0.4276,0.5163,0.6089,1.0000),
                                  labels=c("low_rural_fatal","med_low_rural_fatal","med_high_rural_fatal","high_rural_fatal"))

summary(df2.cat$acc_COUNTY_bin)
# low_rural_fatal  med_low_rural_fatal med_high_rural_fatal     high_rural_fatal                 NA's 
#           14993                13872                14113                14648                  23

#replace NA's with category of "not known"
df2.cat$acc_COUNTY_bin<-df2.cat$acc_COUNTY_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$acc_COUNTY_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("acc_COUNTY","Rural","Urban", "Var1", "total", "per_rur"))]
...
df2.cat$veh_REG_STAT_nm_bin<-cut(df2.cat$per_rur, breaks=c(0.08696,0.43292,0.52032,0.63290,0.90946),
                          labels=c("low_rural_fatal","med_low_rural_fatal","med_high_rural_fatal","high_rural_fatal"))
...
#replace NA's with category of "not known"
df2.cat$veh_REG_STAT_nm_bin<-df2.cat$veh_REG_STAT_nm_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_REG_STAT_nm_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_REG_STAT_nm","Rural","Urban", "Var1", "total", "per_rur"))]
...
#replace NA's with category of "not known"
df2.cat$veh_MAKE_nm_bin<-df2.cat$veh_MAKE_nm_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_MAKE_nm_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_MAKE_nm","Rural","Urban", "Var1", "total", "per_rur"))]
...
#replace NA's with category of "not known"
df2.cat$veh_MODEL_bin<-df2.cat$veh_MODEL_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_MODEL_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_MODEL","Rural","Urban", "Var1", "total", "per_rur"))]
...
#replace NA's with category of "not known"
df2.cat$veh_MAK_MOD_bin<-df2.cat$veh_MAK_MOD_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_MAK_MOD_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_MAK_MOD","Rural","Urban", "Var1", "total", "per_rur"))]
...
#replace NA's with category of "not known"
df2.cat$veh_BODY_TYP_nm_bin<-df2.cat$veh_BODY_TYP_nm_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_BODY_TYP_nm_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_BODY_TYP_nm","Rural","Urban", "Var1", "total", "per_rur"))]
...
#replace NA's with category of "not known"
df2.cat$veh_L_STATE_nm_bin<-df2.cat$veh_L_STATE_nm_bin %>% fct_explicit_na(na_level = 'Not Known')
levels(df2.cat$veh_L_STATE_nm_bin)

#Remove original variable and all other calc variables included in merged table
df2.cat<-df2.cat[, -which(names(df2.cat) %in% c("veh_L_STATE_nm","Rural","Urban", "Var1", "total", "per_rur"))]
...
# 1923-2000 2001-2005 2006-2011 2011-2019      NA's 
#     14914     17151     13686     12347       239
...
#add key back to dataframe for future use
df2.cat.key<-cbind(df2.cat, df2_key)
nrow(df2.cat.key)
#[1] 57862
ncol(df2.cat.key)
#[1] 204
saveRDS(df2.cat.key, "df2_cat_key.rds")
...
############################
df.cont<-df2[,c( "acc_PEDS", "acc_PERNOTMVIT", "acc_VE_TOTAL",
...
ddply(df2,~ veh_prev_sus,summarise,number_of_distinct_orders=length(unique(ï..Key)))
...
##negative numbers indicate unknown
ddply(df2,~ oth_veh_prev_sus,summarise,number_of_distinct_orders=length(unique(ï..Key)))
#1088 unknown.  Assign to most frequent value 0.
...
##add key back into dataframe
df.cont.key<-cbind(df.cont, df2_key)
nrow(df.cont.key)
#[1] 57862
ncol(df.cont.key)
#[1] 24
saveRDS(df.cont.key,"df_cont_key.rds")

#############################
#Build Table that holds categorical and continuous variables
#############################
#merge the categorical and continuous data frames
df2.merge<-cbind(df2.cat, df.cont)
nrow(df2.merge)
#57862
ncol(df2.merge)
#226
...
write.csv(names(df2.merge),"3-Data Exploration/names_df2_merge.csv")
...
corr<-assoc(df2.merge,method = c("auto"))

#write values to file
write.csv(corr$value, "3-Data Exploration/corr_value.csv")

#write p-value to file
write.csv(corr$p.value, "3-Data Exploration/corr_p_value.csv")

#write type (method) to file
write.csv(corr$type, "3-Data Exploration/corr_type.csv")
...
##need to check if same is true of svm and random k-nearest neighbor

#############################
##add to the three resulting rds files
##the key field
#############################
...
##add key back to dataframe
df2.merge.key<-cbind(df2.merge, df2_key)
nrow(df2.merge.key)
#[1] 57862
ncol(df2.merge.key)
#[1] 227

#save file as .rds for later use
saveRDS(df2.merge.key, "df2_merge_key.rds")